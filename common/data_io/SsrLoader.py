...
def readSsrData(filename, startFrame, nFrames, convertToMM=True):
    ...
    myCamParams = Data.CameraParameters(
    ...
    ssrFixup = False
    if binFrameLength != (
        myXMLParser.getFrameLengthDepthMap() + 8
    ):  # 4 bytes CRC, 4 bytes length (tail)
    ...
    # skip frames
    logging.debug(
        "Skipping first %u frames, each has a length of %u bytes:",
        startFrame,
        skipLength,
    )
    ...
    for i in range(0, nFrames, 1):
        ...
        if version[0] == 2:
            ...
            (frameNumber, quality, status) = unpack_from(
                "<IBB", file.read(format2BlockSize)
            )
        ...
        distData.append(
            np.reshape(np.frombuffer(buffer_dist, distType), (nRows, nCols))
        )
        intsData.append(
            np.reshape(np.frombuffer(buffer_ints, intsType), (nRows, nCols))
        )
        if cnfiType != None:
            cnfiData.append(
                np.reshape(np.frombuffer(buffer_cnfi, cnfiType), (nRows, nCols))
            )
        else:
            cnfiData = None
