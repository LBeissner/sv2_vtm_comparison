...
class Streaming:
    ...
    def __init__(self, ipAddress="192.168.1.10", tcpPort=2114):  # 192.168.1.10
    ...
    def openStream(self):
        ...
        try:
            ...
            print(
                "Error on connecting to %s:%d: %s" % (self.ipAddress, self.tcpPort, err)
            )
            logger.error(
                "Error on connecting to %s:%d: %s" % (self.ipAddress, self.tcpPort, err)
            )
    ...
    def getFrame(self, peek=False):
        ...
        try:
            ...
            if receiveLenth < BLOB_HEAD_LEN:
                raise socket.error(
                    "Network connection closed by peer. Receive length is {} and should be {}".format(
                        receiveLenth, BLOB_HEAD_LEN
                    )
                )
        ...
        assert (
            len(header) == BLOB_HEAD_LEN
        ), "Uh, not enough bytes for BLOB_HEAD_LEN, only %s" % (len(header))

        # check if the header content is as expected
        (magicword, pkgLength, protocolVersion, packetType) = struct.unpack(
            ">IIHB", header
        )
        ...
        while toread:
            ...
            if nBytes == 0:
                ...
                raise RuntimeError(
                    "received {} but requested {} bytes".format(
